...
class EmprestimoService:
    ...
    def renovar_emprestimo(self, id_emprestimo: int) -> Emprestimos:
        ...
        if emprestimo.data_devolucao_real is not None:
            raise ValueError("Este empréstimo já foi devolvido. Não é possível renovar.")
    ...
    def registrar_devolucao(self, id_emprestimo: int) -> dict:
        ...
        # Registra a devolução SEM apagar o prazo original (data_devolucao
        # continua sendo a data prevista; data_devolucao_real guarda quando
        # a devolução de fato aconteceu).
    ...
    def emprestimo_atual_por_usuario(self, id_usuario: int) -> Optional[Emprestimos]:
        """Retorna o empréstimo em andamento do usuário, caso exista.
        Usado pelo Aluno ('Meus Empréstimos') e pelo Bolsista (Central
        de Empréstimos, para saber o que renovar)."""
        if id_usuario <= 0:
            raise ValueError("O ID deve ser um número inteiro positivo.")
        return self.repositorio_emprestimo.buscar_atual_por_usuario(id_usuario)
