...
def mostrar_interface_bibliotecario(root, usuario, usuario_service,
    ...
    titulos_lista = livro_service.listar_titulos() if livro_service else []
    ...
    montar_card_stat(frame_cards, len(titulos_lista),   "Títulos no Acervo",       COR_TEXTO_ESCURO)
    ...
    def atualizar_cards():
        """Recalcula e redesenha os 4 cards de estatística do topo."""
        for w in frame_cards.winfo_children():
            w.destroy()
        titulos = livro_service.listar_titulos() if livro_service else []
        todos = livro_service.listar_livros() if livro_service else []
        indisp = livro_service.listar_indisponiveis() if livro_service else []
        atras = emprestimo_service.listar_emprestimos_atrasados() if emprestimo_service else []
        disp = [l for l in todos if l.disponivel]

        montar_card_stat(frame_cards, len(titulos), "Títulos no Acervo",      COR_TEXTO_ESCURO)
        montar_card_stat(frame_cards, len(disp),   "Exemplares Disponíveis", "#2a6a4a")
        montar_card_stat(frame_cards, len(indisp), "Empréstimos Ativos",     "#2a4a8a")
        montar_card_stat(frame_cards, len(atras),  "Em Atraso",              COR_VERMELHO)

    def atualizar_pagina():
        """Atualiza os cards de estatística e recarrega a aba do Acervo."""
        atualizar_cards()
        aba_acervo()
    ...
    def aba_acervo():
        ...
        # Busca
        frame_busca = tk.Frame(topo, bg=COR_FUNDO_INTERNO)
        frame_busca.pack(side="left")

        e_busca = tk.Entry(frame_busca, font=FONTE_CAMPO, relief="flat",
                           bg="white", fg="#888888", bd=6, width=36)
        e_busca.insert(0, "Buscar por título, autor ou ISBN...")
        e_busca.pack(side="left", ipady=5)

        botao_secundario(frame_busca, "🔍 Pesquisar",
                          lambda: atualizar_tabela(e_busca.get())
        ).pack(side="left", padx=(8, 0))

        # Ações (direita)
        frame_acoes = tk.Frame(topo, bg=COR_FUNDO_INTERNO)
        frame_acoes.pack(side="right")

        botao_principal(frame_acoes, "+ Adicionar Livro",
                        lambda: popup_adicionar_livro(area, atualizar_pagina, livro_service)
        ).pack(side="right")

        botao_secundario(frame_acoes, "🔄 Atualizar", atualizar_pagina
        ).pack(side="right", padx=(0, 8))

        # Rodapé com ações — fixado na base ANTES da tabela expandir,
        # assim nunca fica escondido fora da janela.
        rodape = tk.Frame(area, bg=COR_FUNDO_INTERNO)
        rodape.pack(side="bottom", fill="x", pady=6)
        ...
        # Ações do rodapé (Editar / Excluir)
        def remover():
            sel = tree.selection()
            if not sel:
        ...
        def editar():
            ...
            if livro:
                popup_editar_livro(area, livro, atualizar_pagina, livro_service)

        def novo_exemplar():
            sel = tree.selection()
            if not sel:
                messagebox.showwarning("Aviso", "Selecione um livro para adicionar outro exemplar.")
                return
            try:
                id_livro = int(sel[0])
                livro_service.adicionar_exemplar(id_livro)
                messagebox.showinfo("Sucesso", "Novo exemplar adicionado ao acervo.")
                atualizar_pagina()
            except ValueError as e:
                messagebox.showerror("Erro", str(e))

        botao_secundario(rodape, "✏  Editar Selecionado", editar).pack(side="left", padx=(0, 8))
        botao_secundario(rodape, "📄  + Exemplar", novo_exemplar).pack(side="left", padx=(0, 8))
        botao_secundario(rodape, "🗑  Excluir Selecionado", remover).pack(side="left")
    ...
    def aba_emprestimos():
        ...
        ativos = [e for e in todos_emp if e.data_devolucao_real is None]
            ...
            for u in usuarios_enc:
                ...
                for emp in emprestimos:
                    ...
                    tree.insert("", tk.END, values=(
                        ...
                        l.titulo if l else f"#{emp.id_livro}",
                        emp.data_emprestimo,
                        emp.data_devolucao_real if emp.data_devolucao_real else "Em andamento",
                        "Sim" if emp.renovado else "Não",
                    ))
...
def popup_adicionar_livro(pai, ao_salvar, livro_service=None):
    ...
    tk.Label(corpo, text="QUANTIDADE DE EXEMPLARES *", font=("Arial", 9, "bold"),
             bg="white", fg="#333333").pack(anchor="w", pady=(12, 2))
    e_qtd = tk.Entry(corpo, font=("Arial", 11), relief="solid", bd=1)
    e_qtd.insert(0, "1")
    e_qtd.pack(fill="x", ipady=6)
    ...
    def salvar():
        ...
        for k, v in vals.items():
            if v in placeholders:
                vals[k] = ""
        try:
            qtd_texto = e_qtd.get().strip()
            if not qtd_texto.isdigit() or int(qtd_texto) < 1:
                raise ValueError("Informe uma quantidade de exemplares válida (número inteiro ≥ 1).")
            quantidade = int(qtd_texto)

            if livro_service:
                livro_service.cadastrar_livro(vals["titulo"], vals["autor"], vals["genero"], vals["isbn"], quantidade)
            popup.destroy()
