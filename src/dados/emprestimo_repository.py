...
class EmprestimosRepository:
    ...
    def listar_todos(self) -> list[Emprestimos]:
        ...
        cursor.execute(
            """
            SELECT id, id_usuario, id_livro, data_emprestimo, data_devolucao,
                   data_devolucao_real, renovado
            FROM emprestimos
            ORDER BY id
            """
    ...
    def listar_ativos_por_usuario(self, id_usuario: int) -> list[Emprestimos]:
        ...
        'Não devolvido' agora é confiável: data_devolucao_real IS NULL."""
        cursor = self.conexao.cursor(dictionary=True)
    ...
    def listar_atrasados(self, data_hoje: str) -> list[Emprestimos]:
        ...
        cursor.execute(
            """
            SELECT e.id, e.id_usuario, e.id_livro, e.data_emprestimo, e.data_devolucao,
                   e.data_devolucao_real, e.renovado
            FROM emprestimos e
    ...
    def buscar_atual_por_usuario(self, id_usuario: int) -> Optional[Emprestimos]:
        """Retorna o empréstimo em andamento (ainda não devolvido) de um
        usuário, caso exista. Usado na Central de Empréstimos (bolsista)
        e em 'Meus Empréstimos' (aluno)."""
        cursor = self.conexao.cursor(dictionary=True)
    ...
    def tem_emprestimo_ativo(self, id_livro: int) -> bool:
        """Verifica se o exemplar tem empréstimo ativo (não devolvido). Usado em US14."""
        cursor = self.conexao.cursor()

        cursor.execute(
            """
            SELECT COUNT(*) FROM emprestimos
            WHERE id_livro = %s AND data_devolucao_real IS NULL
            """,
            (id_livro,),
        )
    ...
    def _mapear(linha: dict) -> Emprestimos:
        return Emprestimos(
            ...
            data_devolucao_real=(
                str(linha["data_devolucao_real"]) if linha.get("data_devolucao_real") else None
            ),
        )